...
command_asgn	: lhs '=' lex_ctxt command_rhs
		...
		| defn_head f_opt_paren_args(do_HIGHEST) '=' command(do_HIGHEST)
		    {
		...
		| defn_head f_opt_paren_args(do_HIGHEST) '=' command(do_HIGHEST) modifier_rescue arg(do_HIGHEST)
		    {
			endless_method_name(p, $<node>1, &@1);
			restore_defun(p, $<node>1->nd_defn);
		    /*%%%*/
		...
		| defs_head f_opt_paren_args(do_HIGHEST) '=' command(do_HIGHEST)
		    {
			endless_method_name(p, $<node>1, &@1);
			restore_defun(p, $<node>1->nd_defn);
		    /*%%%*/
		...
		| defs_head f_opt_paren_args(do_HIGHEST) '=' command(do_HIGHEST) modifier_rescue arg
		    {
			endless_method_name(p, $<node>1, &@1);
			restore_defun(p, $<node>1->nd_defn);
		    /*%%%*/
...
arg		: lhs '=' lex_ctxt arg_rhs
		...
		| defn_head f_opt_paren_args(do_HIGHEST) '=' arg(do_HIGHEST)
		    {
			endless_method_name(p, $<node>1, &@1);
			restore_defun(p, $<node>1->nd_defn);
		    /*%%%*/
		...
		| defn_head f_opt_paren_args(do_HIGHEST) '=' arg(do_HIGHEST) modifier_rescue arg(do_HIGHEST)
		    {
			endless_method_name(p, $<node>1, &@1);
			restore_defun(p, $<node>1->nd_defn);
		    /*%%%*/
		...
		| defs_head f_opt_paren_args(do_HIGHEST) '=' arg(do_HIGHEST)
		    {
			endless_method_name(p, $<node>1, &@1);
			restore_defun(p, $<node>1->nd_defn);
		    /*%%%*/
		...
		| defs_head f_opt_paren_args(do_HIGHEST) '=' arg(do_HIGHEST) modifier_rescue arg(do_HIGHEST)
		    {
			endless_method_name(p, $<node>1, &@1);
			restore_defun(p, $<node>1->nd_defn);
		    /*%%%*/
...
parse_ident(struct parser_params *p, int c)
{
	...
	if (kw) {
		...
		if (CMDARG_P() && !IS_lex_state_for(state, EXPR_CMDARG))
		    return keyword_do; /* keyword_do_block */
		return keyword_do;
	    }
