  ...
  describe '#parse' do
    ...
    context 'when parameterized rules' do
      ...
      context "when user defined rules" do
        ...
        context "when nested rules in rhs with named ref" do
          let(:path) { "parameterized/user_defined/nested_rules_in_rhs_with_named_ref.y" }

          it "expands parameterized rules" do
            expect(grammar.nterms.sort_by(&:number)).to match_symbols([
              Sym.new(id: T::Ident.new(s_value: "$accept"), alias_name: nil, number: 5, tag: nil, term: false, token_id: 0, nullable: false),
              Sym.new(id: T::Ident.new(s_value: "option_number"), alias_name: nil, number: 6, tag: nil, term: false, token_id: 1, nullable: true),
              Sym.new(id: T::Ident.new(s_value: "pair_number_number"), alias_name: nil, number: 7, tag: T::Tag.new(s_value: "<i>"), term: false, token_id: 2, nullable: false),
              Sym.new(id: T::Ident.new(s_value: "program"), alias_name: nil, number: 8, tag: nil, term: false, token_id: 3, nullable: false),
            ])
            ...
            expect(grammar.rules).to eq([
              ...
              Rule.new(
                ...
                lhs_tag: T::Tag.new(s_value: "<i>"),
                token_code: T::UserCode.new(s_value: " $$ = $x + $y; "),
                nullable: false,
                precedence_sym: grammar.find_symbol_by_s_value!("'+'"),
                lineno: 25,
              ),
    ...
    describe "invalid prec" do
      it "raises error if code exists after %prec" do
        ...
        expect { parser.parse }.to raise_error(ParseError, <<~ERROR.chomp)
          parse.y:31.78-79: error: multiple User_code after %prec
      ...
      it "raises error if ident appears after %prec, and it is intermediate" do
        ...
        expect { parser.parse }.to raise_error(ParseError, <<~ERROR.chomp)
          parse.y:31.34-45: error: intermediate %prec in a rule
      ...
      it "raises error if char appears after %prec, and it is intermediate" do
        ...
        expect { parser.parse }.to raise_error(ParseError, <<~ERROR.chomp)
          parse.y:31.32-43: error: intermediate %prec in a rule
      ...
      it "raises error if multiple %prec in a rule" do
        ...
        expect { parser.parse }.to raise_error(ParseError, <<~ERROR.chomp)
          parse.y:31.40-46: error: multiple %prec in a rule
      ...
      describe "" do
        ...
        context "includes invalid named references" do
          it "raise an error" do
            ...
            expected = <<~ERROR.chomp
              parse.y:27.18-26: error: Referring symbol `results` is not found.
    ...
    describe "error messages" do
      context "error_value has line number and column" do
        it "contains line number and column" do
          ...
          expect { parser.parse }.to raise_error(ParseError, <<~ERROR.chomp)
            error_messages/parse.y:5.8-15: error: parse error on value 'invalid' (IDENTIFIER)
                5 | %expect invalid
                  |         ^~~~~~~
          ERROR
        end
      ...
      context "error_value doesn't have line number and column" do
        it "contains line number and column" do
          ...
          expect { parser.parse }.to raise_error(ParseError, <<~ERROR.chomp)
            error_messages/parse.y:5.10-12: error: parse error on value 10 (INTEGER)
                5 | %expect 0 10
                  |           ^~
          ERROR
        end
      ...
      context "line of invalid syntax contains tab" do
        it "correctly mark the location of an error" do
          ...
          expect { parser.parse }.to raise_error(ParseError, <<~ERROR.chomp)
            error_messages/parse.y:5.9-16: error: parse error on value 'invalid' (IDENTIFIER)
                5 | %expect\t\tinvalid
                  |        \t\t^~~~~~~
          ERROR
        end
      ...
      context "when pass the terminal symbol to `%nterm`" do
        it "raises an error" do
          ...
          expect { parser.parse }.to raise_error(ParseError, <<~ERROR.chomp)
            error_messages/parse.y:6.7-10: error: symbol EOI redeclared as a nonterminal
                6 | %nterm EOI
                  |        ^~~
          ERROR
        end
      ...
      context "when multiple %empty" do
        it "raises an error" do
          ...
          expect { parser.parse }.to raise_error(ParseError, <<~'ERROR'.chomp)
            error_messages/parse.y:7.9-15: error: %empty on non-empty rule
      ...
      context "when symbol after %empty" do
        it "raises an error" do
          ...
          expect { parser.parse }.to raise_error(ParseError, <<~'ERROR'.chomp)
            error_messages/parse.y:9.16-22: error: %empty on non-empty rule
