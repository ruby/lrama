...
RSpec.describe Lrama::Diagnostics::Color do
  ...
  describe '.for_diagnostic' do
    before { described_class.enabled = true }

    it 'colorizes text with diagnostic type' do
      result = described_class.for_diagnostic('message', :error)
      expect(result).to include("\e[")
  ...
  describe 'CODES' do
    ...
    it 'includes style codes' do
      ...
      expect(described_class::CODES[:underline]).to eq("\e[4m")
    end
  end
